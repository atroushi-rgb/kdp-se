...
$config_check_referer                = FALSE; // Set to TRUE for more seciruty
//#################
...
if($action == "logout")
{
    ...
    msg("info", "Logout", "You are now logged out, <a href=\"http://www.kdp.se/admin.php\">login agin</a> or go to <a href=\"http://www.kdp.se\">www.kdp.se</a><br /><br>");
}
...
if(substr($HTTP_REFERER, -1) == "/"){ $HTTP_REFERER.= "admin.php"; }
...
if($config_check_referer == TRUE){
    ...
    if(!eregi("$self",$HTTP_REFERER) and $HTTP_REFERER != ""){
            die("<h2>Sorry but your access to this page was denied !</h2><br>try to <a href=\"?action=logout\">logout</a> and then login again<br>To turn off this security check, change \$config_check_referer in admin.php to FALSE");
        }
}
// ********************************************************************************
